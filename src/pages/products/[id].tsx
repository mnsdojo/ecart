...
function ProductPage({ product }: { product: ProductCardType }) {
  const [count, setCount] = useState(0);
  const { addItem } = useShoppingCart();
  const addToCart = (e: any) => {
    e.preventDefault();
    const id = toast.loading(`Adding ${count} item${count > 1 ? "s" : ""}...`);
    addItem(product as any, { count });
    ...
    <div className="container lg:max-w-screen-lg mx-auto py-12 px-6">
      <div className="flex flex-col md:flex-row justify-between items-center space-y-8 md:space-y-0 md:space-x-12">
        ...
        <div className="w-full flex-1 max-w-md border border-opacity-50 rounded-md shadow-lg p-6 bg-white">
          ...
          <button
            onClick={addToCart}
            className="w-full mt-4 disabled:cursor-not-allowed disabled:opacity-50  rounded hover:text-white border py-2 px-6 hover:bg-black/[0.9]"
          >
            Add to Cart
          </button>
        </div>
